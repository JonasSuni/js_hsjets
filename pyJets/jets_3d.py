...
def generate_cmap_plots(cmap_axes, vobj, x0, y0, z0, limitedsize):
    ...
    x_arr = polyval_bs_at_time(fnr, y_arr, z_arr, ms=True)
    cmap_axes[0].plot(x_arr, y_arr, linewidth=0.6, color="red")
    ...
    x_arr = polyval_bs_at_time(fnr, y_arr, z_arr, ms=True)
    cmap_axes[1].plot(x_arr, z_arr, linewidth=0.6, color="red")
    ...
    xmesh = polyval_bs_at_time(fnr, ymesh, zmesh, ms=True)
    cmap_axes[2].contour(ymesh, zmesh, xmesh, [x0], linewidths=[0.6], colors=["red"])
...
def plot_bs_deflection(runid="FIF", interpolate=True):
    ...
    for fnr in fnr_arr:
        ...
        fig.savefig(outdir + "/{}.png".format(fnr), dpi=300, bbox_inches="tight")
        plt.close(fig)
        print("Plotted bow shock deflection for fnr {}".format(fnr))
...
def plot_bs_mp_map_all(runid="FIF"):
    ...
    for fnr in fnr_arr:
        ...
        ax_list[0].plot(ms_x_of_y, y_arr, ".", color="k")
        ax_list[0].plot(ms_x_of_y_fit, y_arr, color="k")

        ax_list[0].plot(rho_x_of_y, y_arr, ".", color="red")
        ax_list[0].plot(rho_x_of_y_fit, y_arr, color="red")

        ax_list[0].plot(mp_x_of_y, y_arr, ".", color="k")
        ax_list[0].plot(mp_x_of_y_fit, y_arr, color="k")

        ax_list[1].plot(ms_x_of_z, z_arr, ".", color="k")
        ax_list[1].plot(ms_x_of_z_fit, z_arr, color="k")

        ax_list[1].plot(rho_x_of_z, z_arr, ".", color="red")
        ax_list[1].plot(rho_x_of_z_fit, z_arr, color="red")

        ax_list[1].plot(mp_x_of_z, z_arr, ".", color="k")
...
def plot_detailed_trace(cellid, tstart_list, runid="FIF", plot_every=1):
    ...
    time_arr_all = np.arange(tmin, tmax + 0.01, 0.1)

    for idx in range(time_arr_all.size):
        t = np.round(time_arr_all[idx], 1)
        print(t)
...
class GMM:
    ...
    def MSTEP(self, X, sample_weights, member_probabilities):
        ...
        for idx in range(self.nMaxwellians):
            ...
            new_mean = np.sum(
                sample_weights[:, None] * member_probabilities[:, idx][:, None] * X,
                axis=0,
        ...
        if histogram:
            ...
            if gmm:
                for idx in range(gmm):
                    ...
                    ax_list[0].plot(
                        np.nanmean(x[::plot_every][mask] / r_e),
