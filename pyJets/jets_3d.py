...
def L3_good_timeseries_global_vdfs_one(idx, limitedsize=True, n_processes=16):
    ...
    for idx, fnr in enumerate(fnr_range):
        args_list.append((cellids[idx], coords, t0[idx], t1[idx], fnr, limitedsize, outdir))
    ...
    subprocess.run(
        "cat $(find {} -maxdepth 1 -name '*.png' | sort -V) | ffmpeg -framerate 5 -i - -pix_fmt yuv420p -crf 18 -vf scale=1920:-2 -y {}".format(
            outdir, outfilename
        ),
        shell=True,
    )
