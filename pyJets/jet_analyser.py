...
def sw_par_dict(runid):
    ...
    runs = ["ABA", "ABC", "AEA", "AEC", "BFD", "AGF", "AIA", "AIC"]
    sw_rho = [1e6, 3.3e6, 1.0e6, 3.3e6, 1.0e6, 1.0e6, 1.0e6, 1.0e6]
    sw_v = [750e3, 600e3, 750e3, 600e3, 750e3, 750e3, 750e3, 750e3]
    sw_B = [5.0e-9, 5.0e-9, 10.0e-9, 10.0e-9, 5.0e-9, 3.0e-9, 3.0e-9, 3.0e-9]
    sw_T = [500e3, 500e3, 500e3, 500e3, 500e3, 500e3, 500e3, 500e3]
...
def mask_maker(runid, filenr, boxre=[6, 18, -8, 6], avgfile=True, mag_thresh=1.5):
    ...
    if type(vlsvreader.read_variable("rho")) is not np.ndarray:
        ...
        pr_PTDNBS = vlsvreader.read_variable("PTensorNonBackstreamDiagonal")[
            np.argsort(origid)
        ]
        proton_bool = True
    else:
...
def make_cust_mask_opt(
    ...
    if vlsvreader.check_variable("X"):
        X, Y, Z = [
            vlsvreader.read_variable("X"),
            vlsvreader.read_variable("Y"),
            vlsvreader.read_variable("Z"),
        ]
    else:
        X, Y, Z = xyz_reconstruct(vlsvreader)

    X, Y, Z = [
        X[np.argsort(origid)] / r_e,
        Y[np.argsort(origid)] / r_e,
        Z[np.argsort(origid)] / r_e,
    ]
    ...
    if not not boxre:
        ...
        # print(masked_ci[69])
        if transient == "slamsjet":
            masked_ci_jet = np.intersect1d(
                masked_ci_jet, restrict_area(vlsvreader, boxre)
            )
            masked_ci_slams = np.intersect1d(
                masked_ci_slams, restrict_area(vlsvreader, boxre)
            )
            np.savetxt(
                "{}working/{}Masks/{}/".format(wrkdir_DNR, "jets/", runid)
                + str(filenumber)
