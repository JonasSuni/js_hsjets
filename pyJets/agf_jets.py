            ...
            try:
                ...
                if len(x_points.shape) == 1:
                    ...
                    for xp in x_points:
                        ...
                        ez = (
                            vobj.read_variable(
                                "vg_e_vol", operator="z", cellids=vobj.get_cellid(xp)
                            )
                            / 1e-3
                        )
                        ax.plot(
...
def fig1_new(
    ...
    cbax = fig.add_axes((0.85, 0.2, 0.03, 0.5))
...
def plot_jet_formation_postime(
    ...
    fig.savefig(figdir + "fig2.pdf", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def supp_vdf_figure():
    ...
    fig.suptitle("X = {}, Y = {} ".format(x, y) + "$R_\\mathrm{E}$", fontsize=24)
...
def pos_vdf_plotter(
    ...
    global runid_g, sj_ids_g, non_ids_g, filenr_g, Blines_g, x0, y0, plaschke_g, drawBy0, linsg, draw_qperp, leg_g, highres_g, chg, bsg, mmsg
    drawBy0 = True
    draw_qperp = False
    plaschke_g = False
    runid_g = runid
