...
def hack_2019_fig6_alt(time_thresh=5):
    ...
    for col in range(3):
        ...
        leg = ax.legend(handles[::-1],labels[::-1],fontsize=20,frameon=False,markerscale=0.5)
        jx.legend_compact(leg)
...
def rev1_jetpath(runid,vavgfilename,time_thresh=5,crop=True):
    ...
    vlsvobj = pt.vlsvfile.VlsvReader("/scratch/project_2000506/sunijona/bulks/"+runid+"/bulk.0001000.vlsv")
...
def rev1_defplot(time_thresh=5,type="angmag"):
    ...
    ax_list[0].annotate("a)",xy=(0.02,0.07),xycoords="axes fraction",color="black",fontsize=20)
...
def h19_movie(runid,start,stop):
    ...
    for n in range(start,stop+1):
        ...
        pt.plot.plot_colormap(filename=bulkpath+bulkname,outputfile=outputdir+"zoom/"+str(n)+".png",usesci=0,lin=1,boxre=[10,14,-3,1],expression=pc.expr_pdyn,vmin=0,vmax=vmax,colormap="parula",cbtitle="nPa",external=h19_fig1_ext,pass_vars=["rho","v","CellID","Pdyn"],tickinterval=1.0)
