...
def plot_new_sj():
    ...
    pt.plot.plot_colormap(
        filename="/wrk/group/spacephysics/vlasiator/2D/ABC/bulk/bulk.0000677.vlsv",
        outputdir=wrkdir_DNR + "Plots/",
        usesci=0,
        lin=1,
        boxre=[6, 18, -8, 6],
        var="B",
...
def jet_plotter(start, stop, runid, vmax=1.5, boxre=[6, 18, -8, 6], transient="jet"):
    ...
    for n in range(start, stop + 1):
        ...
        for itr in range(3000):
            ...
            try:
                props = jio.PropReader(
                    str(itr).zfill(5), runid, 580, transient=transient
                )
        ...
        try:
            full_cells = np.loadtxt(
                inputpath + "Masks/{}/{}.mask".format(runid, n)
            ).astype(int)
        except IOError:
            full_cells = []
...
def ext_jet(ax, XmeshXY, YmeshXY, pass_maps):
    ...
    jet_cont = ax.contour(
        XmeshXY, YmeshXY, jet_mask, [0.5], linewidths=0.8, colors="black"
    )  # Contour of jets

    (line1,) = ax.plot(
        xmean_list, ymean_list, "o", color="red", markersize=2
    )  # Mean positions
    (line2,) = ax.plot(
        xvmax_list, yvmax_list, "o", color="white", markersize=2
    )  # v_max positions
...
def slamjet_plotter(start, stop, runid, vmax=1.5, boxre=[6, 18, -8, 6]):
    ...
    for n in range(start, stop + 1):
        ...
        for itr in range(1, maxrange[rid_list.index(runid)] + 1):
            ...
            try:
                props = jio.PropReader(
                    str(itr).zfill(5), runid, 580, transient="slamsjet"
                )
                xmean_list.append(props.read_at_time("x_mean", float(n) / 2))
                ymean_list.append(props.read_at_time("y_mean", float(n) / 2))
            except IOError:
        ...
        try:
            fileobj = open(
                wrkdir_DNR + "working/SLAMSJETS/events/{}/{}.events".format(runid, n),
                "r",
            )
            contents = fileobj.read()
            fileobj.close()
...
class MMSJet:
    ...
    def read_mftime(self):
        time = (
            self.mfdata[:, self.mfvar_list.index("Hour")]
            + self.mfdata[:, self.mfvar_list.index("Minute")] / 60.0
            + self.mfdata[:, self.mfvar_list.index("Second")] / 3600.0
        )
        return time
    ...
    def read(self, name, tmin=0, tmax=999):
        if name == "time":
            time = (
                self.jetdata[:, self.jetvar_list.index("Hour")]
                + self.jetdata[:, self.jetvar_list.index("Minute")] / 60.0
                + self.jetdata[:, self.jetvar_list.index("Second")] / 3600.0
            )
            ...
            if name in ["TParallel", "TPerpendicular"]:
                return (
                    self.jetdata[:, self.jetvar_list.index(name)][b_arr] * eVtoK * 1e-6
                )
            elif name == "Pdyn":
            ...
            intpol_data = np.interp(
                self.read("time", tmin=tmin, tmax=tmax), self.read_mftime(), in_data
            )
            return intpol_data
...
class MMSReader:
    def __init__(self, filename):
        ...
        label_list = [
            ...
            "$n_{avg}~[n_{sw}]$",
            "$n_{max}~[n_{sw}]$",
            "$P_{dyn,avg}~[P_{dyn,sw}]$",
            "$P_{dyn,max}~[P_{dyn,sw}]$",
            "$T_{\\parallel,avg}~[T_{sw}]$",
            "$T_{\\perp,avg}~[T_{sw}]$",
            "$T_{avg}~[T_{sw}]$",
            "$|V|_{avg}~[V_{sw}]$",
            "$|V|_{avg}~[V_{sw}]$",
...
def sheath_pars_list(var):
    ...
    label_list = [
        "$P_{dyn,vmax}~[P_{dyn,sh}]$",
        "$P_{dyn,avg}~[P_{dyn,sh}]$",
        "$P_{dyn,med}~[P_{dyn,sh}]$",
        "$P_{dyn,max}~[P_{dyn,sh}]$",
        "$n_{max}~[n_{sh}]$",
        "$n_{avg}~[n_{sh}]$",
        "$n_{med}~[n_{sh}]$",
        "$n_{v,max}~[n_{sh}]$",
    ...
    norm_list = [0.25, 0.25, 0.25, 0.25, 4, 4, 4, 4, 0.25, 0.25, 0.25]
...
def var_pars_list(var):
    ...
    label_list = [
        "$Lifetime~[s]$",
        "$Radial~size~[R_{e}]$",
        "$Tangential~size~[R_{e}]$",
        "$Radial~size/Tangential~size$",
        "$P_{dyn,vmax}~[P_{dyn,sw}]$",
        "$P_{dyn,avg}~[P_{dyn,sw}]$",
        "$P_{dyn,med}~[P_{dyn,sw}]$",
        "$P_{dyn,max}~[P_{dyn,sw}]$",
        "$n_{max}~[n_{sw}]$",
        "$n_{avg}~[n_{sw}]$",
        "$n_{med}~[n_{sw}]$",
        ...
        "$B_{avg}~[B_{IMF}]$",
        "$B_{med}~[B_{IMF}]$",
        "$\\beta _{max}~[\\beta _{sw}]$",
        "$\\beta _{avg}~[\\beta _{sw}]$",
        "$\\beta _{med}~[\\beta _{sw}]$",
        "$\\beta _{v,max}~[\\beta _{sw}]$",
        "$T_{avg}~[T_{sw}]$",
        "$T_{med}~[T_{sw}]$",
        "$T_{max}~[T_{sw}]$",
        "$T_{Parallel,avg}~[T_{sw}]$",
        "$T_{Parallel,med}~[T_{sw}]$",
        "$T_{Parallel,max}~[T_{sw}]$",
        "$T_{Perpendicular,avg}~[T_{sw}]$",
        "$T_{Perpendicular,med}~[T_{sw}]$",
        "$T_{Perpendicular,max}~[T_{sw}]$",
        "$Area~[R_{e}^{2}]$",
    ...
    return [
        label_list[key_list.index(var)],
        xmin_list[key_list.index(var)],
        xmax_list[key_list.index(var)],
        step_list[key_list.index(var)],
        tickstep_list[key_list.index(var)],
    ]
...
def jet_pos_graph(runid):
    ...
    for fname in propfiles:
        props = pd.read_csv(
            wrkdir_DNR + "working/jets/jets/" + runid + "/" + fname
        ).as_matrix()
        r = props[:, 6]
    ...
    for n in range(0, 3000):
        for runid in runids:
            try:
                props = jio.PropReader(str(n).zfill(5), runid, 580, transient="jet")
                if (
                    "splinter" not in props.meta
                    and max(props.read("r_mean")) > run_cutoff_dict[runid]
                ):
            ...
            try:
                props = jio.PropReader(
                    str(n).zfill(5), runid, 580, transient="slamsjet"
                )
...
def jet_paper_counter():
    ...
    for filenames in filenames_list:
        file_list_list.append(
            [filename for filename in filenames if ".props" in filename]
        )
    ...
    for n in range(len(runids)):
        for fname in file_list_list[n]:
            ...
            # Conditions
            if (
                props.read("time")[-1] - props.read("time")[0] + 0.5 > 5
                and max(props.read("r_mean")) > run_cutoff_dict[runids[n]]
            ):
...
def jet_paper_pos():
    ...
    run_color_dict = dict(
        zip(["ABA", "ABC", "AEA", "AEC"], ["black", "red", "blue", "green"])
    )
    ...
    for n in range(len(runids)):
        for fname in file_list_list[n]:
            ...
            # Conditions
            if (
                props.read("time")[-1] - props.read("time")[0] + 0.5 > 10
                and max(props.read("r_mean")) > run_cutoff_dict[runids[n]]
            ):
    ...
    for n in range(len(runids)):
        (line1,) = ax.plot(
            x_list_list[n],
            y_list_list[n],
            run_marker_dict[runids[n]],
            markeredgecolor=run_color_dict[runids[n]],
            markersize=10,
...
def hack_2019_fig4(columnspacing=-1.0, fontsize=15, time_thresh=5):
    ...
    mms_norm = np.array([1, 1, 1, 1, 1, 1.0e6, 1.0e6])
    bins_list = np.array(
        [
            np.linspace(0, 1.6, 10 + 1),
            np.linspace(1.5, 7.5, 10 + 1),
            np.linspace(0, 1, 10 + 1),
            np.linspace(0, 3, 10 + 1),
            np.linspace(0, 12, 10 + 1),
            np.linspace(0, 15, 10 + 1),
        ]
    )
    ...
    MMS_ext, MMS_n, MMS_v, MMS_pd, MMS_B, MMS_TPerp, MMS_TPar = mms_reader.read_mult(
        var_list
    )
    VLH_ext, VLH_n, VLH_v, VLH_pd, VLH_B, VLH_TPerp, VLH_TPar = read_mult_runs(
        var_list, time_thresh, amax=True
    )
    VLR_ext, VLR_n, VLR_v, VLR_pd, VLR_B, VLR_TPerp, VLR_TPar = read_mult_runs(
        var_list, time_thresh
    )

    darr_list = [
        [VLH_ext, VLR_ext, MMS_ext],
        [VLH_n, VLR_n, MMS_n],
        [VLH_v, VLR_v, MMS_v],
        [VLH_pd, VLR_pd, MMS_pd],
        [VLH_B, VLR_B, MMS_B],
        [VLH_TPerp, VLR_TPerp, MMS_TPerp],
        [VLH_TPar, VLR_TPar, MMS_TPar],
    ]
    ...
    for row in range(6):
        for col in range(3):
            ...
            if col == 0 and row == 5:
                lab = "TPerp\nmed:{:.2f}\nstd:{:.2f}".format(
                    np.median(data_arr), np.std(data_arr, ddof=1)
                )
            else:
                lab = "med:{:.2f}\nstd:{:.2f}".format(
                    np.median(data_arr), np.std(data_arr, ddof=1)
            ...
            if row == 5:
                ...
                if col == 0:
                    lab = "TPar\nmed:{:.2f}\nstd:{:.2f}".format(
                        np.median(data_arr_2), np.std(data_arr_2, ddof=1)
                    )
                else:
                    lab = "med:{:.2f}\nstd:{:.2f}".format(
                        np.median(data_arr_2), np.std(data_arr_2, ddof=1)
...
def hack_2019_fig6(time_thresh=5):
    ...
    if sys.version_info.major == 3:
        mpl.rcParams["axes.prop_cycle"] = mpl.cycler(
            color=[jx.violet, jx.medium_blue, jx.dark_blue, jx.orange]
        )
    elif sys.version_info.major == 2:
    ...
    bins_list = np.array(
        [
            np.linspace(time_thresh, 60, 10 + 1),
            np.linspace(0, 0.5, 10 + 1),
            np.linspace(0, 5, 10 + 1),
        ]
    )

    data_list = read_mult_runs(
        var_list, time_thresh, runids=["ABA", "ABC", "AEA", "AEC"], amax=False
    )
...
def hack_2019_fig6_alt(time_thresh=5):
    ...
    if sys.version_info.major == 3:
        mpl.rcParams["axes.prop_cycle"] = mpl.cycler(
            color=["black", jx.violet, jx.orange, jx.green]
        )
    elif sys.version_info.major == 2:
    ...
    bins_list = np.array(
        [
            np.linspace(time_thresh, 60, 20 + 1),
            np.linspace(0, 0.5, 25 + 1),
            np.linspace(0, 5, 25 + 1),
        ]
    )
    ...
    for col in range(3):
        ...
        if col == 0:
            labs_arr = [
                "{} med:{:.2f}".format(paper_runs_list[itr], med_arr[itr])
                for itr in range(len(var_arr))
            ]
        else:
            labs_arr = [
                "med:{:.2f}".format(med_arr[itr]) for itr in range(len(var_arr))
            ]
        ...
        ax.legend(fontsize=20, frameon=False, markerscale=0.5)
        handles, labels = ax.get_legend_handles_labels()
        leg = ax.legend(
            handles[::-1], labels[::-1], fontsize=20, frameon=False, markerscale=0.5
        )
        jx.legend_compact(leg)
        ax.set_xlabel(label_list[col], fontsize=25)
        ax.set_ylim(0, 0.5)
        ax.annotate(
            panel_label_list[col],
            (0.05, 0.925),
            xycoords="axes fraction",
            fontsize=20,
            weight="bold",
...
def jetcand_vdf(runid):
    ...
    vlsvobj_list = [
        pt.vlsvfile.VlsvReader(bulkpath + "bulk.{}.vlsv".format(str(fn).zfill(7)))
        for fn in fn_list
    ]
    ...
    for fn in fn_list:
        ...
        pt.plot.plot_vdf(
            vlsvobj=vlsvobj_list[fn_list.index(fn)],
            outputdir=outputdir,
            cellids=[cellid],
            run=runid,
            step=fn,
            ...
            bpara=True,
            slicethick=0,
            title=title_list[fn_list.index(fn)],
        )
...
def rev1_jetpath(runid, vavgfilename, time_thresh=5, crop=True):
    ...
    vlsvobj = pt.vlsvfile.VlsvReader(
        "/scratch/project_2000506/sunijona/bulks/" + runid + "/bulk.0001000.vlsv"
    )
    ...
    for n in range(1, 2500):
        ...
        if (
            props.read("duration")[0] < time_thresh
            or max(props.read("r_mean")) < cutoff_list[runids_list.index(runid)]
            or "splinter" in props.meta
        ):
            continue
    ...
    for y_value in plasyrange:
        plasx, plasy = rev1_plasmatracker(
            X, Y, vavgx, vavgy, 15, y_value, boxre, maxt=300
        )
        ax.plot(plasx, plasy, color=jx.orange, zorder=2)
        ...
        try:
            vlsvobj_list.append(
                pt.vlsvfile.VlsvReader(
                    bulkpath + "bulk.{}.vlsv".format(str(n).zfill(7))
                )
            )
        except:
            time.sleep(0.1)
    ...
    for itr in range(start, stop + 1):
        ...
        for ix in range(jet_filenrs.size):
            ...
            if type == "angmag":
                ...
                diffs = np.linalg.norm([vx, vy], axis=0) - np.linalg.norm(
                    [vavgx, vavgy], axis=0
                )
                deflec_angle = np.rad2deg(
                    np.arctan2(np.abs(vy), -vx) - np.arctan2(np.abs(vy), -vavgx)
                )
            elif type == "xy":
                vavgx, vavgy, vavgz = vavgs[time_cells - 1].T
                diffs = vx - vavgx
                ...
                sheath_cells_plus = jx.get_neighs(
                    runid, time_cells, neighborhood_reach=[2, 2, 0]
                )
                sheath_cells = sheath_cells_plus[
                    ~np.in1d(sheath_cells_plus, time_cells)
                ]
                vavgx, vavgy, vavgz = vlsvobj.read_variable("v", cellids=sheath_cells).T
                diffs = np.linalg.norm(
                    [
                        np.median(vx) - np.median(vavgx),
                        np.median(vy) - np.median(vavgy),
                        np.median(vz) - np.median(vavgz),
                    ],
                    axis=0,
                )
                deflec_angle = np.rad2deg(
                    np.median(np.arctan2(np.linalg.norm([vy, vz], axis=0), -vx))
                    - np.median(
                        np.arctan2(np.linalg.norm([vavgy, vavgz], axis=0), -vavgx)
                    )
                )
        ...
        np.savetxt(
            outputdir + "jet_{}_diffs.txt".format(itr),
            np.array([jet_times, jet_diffs, jet_deflecs]).T,
        )
...
def rev1_defplot(time_thresh=5, type="angmag"):
    ...
    for n in range(1, 3000):
        for ix, runid in enumerate(runids):
            ...
            if (
                props.read("duration")[0] < time_thresh
                or max(props.read("r_mean")) < cutoff_dict[runid]
            ):
                continue
            ...
            res_diff_arr = np.interp(
                epoch_arr, xdist_arr, deflec_diff, left=np.nan, right=np.nan
            )
            res_angle_arr = np.interp(
                epoch_arr, xdist_arr, deflec_angle, left=np.nan, right=np.nan
            )
    ...
    for ix in range(len(runids)):
        ax_list[0].plot(
            epoch_arr, diff_mean_list[ix], color=color_list[ix], zorder=3, linewidth=1.5
        )
        ax_list[1].plot(
            epoch_arr,
            angle_mean_list[ix],
    ...
    ax_list[0].annotate(
        "LM30", xy=(0.5, 1.05), xycoords="axes fraction", color=jx.orange, fontsize=20
    )
    ax_list[0].annotate(
        "LM05",
        xy=(0.5 + 0.1, 1.05),
    ...
    ax_list[0].annotate(
        "a)", xy=(0.02, 0.07), xycoords="axes fraction", color="black", fontsize=20
    )
    ax_list[1].annotate(
        "b)", xy=(0.02, 0.9), xycoords="axes fraction", color="black", fontsize=20
    )

    plt.tight_layout()

    fig.savefig(
        wrkdir_DNR + "Figures/hackathon_paper/deflection_plot_{}.png".format(type)
    )
    plt.close(fig)
...
def rev1_jetcone_all(time_thresh=5):
    ...
    bins = np.linspace(-8, 8, 10 + 1)
    bins_cone = np.linspace(-50, 50, 10 + 1)
    ...
    for n in range(1, 3000):
        for ix, runid in enumerate(runids):
            ...
            if (
                props.read("duration")[0] < time_thresh
                or max(props.read("r_mean")) < cutoff_list[ix]
            ):
                continue
    ...
    ax_list[0].annotate(
        "a)", xy=(0.05, 0.9), xycoords="axes fraction", color="black", fontsize=25
    )
    ax_list[1].annotate(
        "b)", xy=(0.05, 0.9), xycoords="axes fraction", color="black", fontsize=25
    )
...
def rev1_jetcone(runids, time_thresh=5):
    ...
    fig.savefig(
        homedir
        + "Figures/hackathon_paper/dusk_til_dawn_{}.png".format("_".join(runids))
    )
    plt.close(fig)
...
def read_mult_runs(
    ...
    for n in range(1, 2500):
        for runid in runids:
            ...
            for var in var_list:
                if amax:
                    data_list[var_list.index(var)].append(
                        props.read_at_amax(var) / ja.sw_normalisation(runid, var)
                    )
                else:
                    data_list[var_list.index(var)].append(
                        props.read_at_randt(var) / ja.sw_normalisation(runid, var)
                    )
...
def DT_comparison(time_thresh=5):
    ...
    hist_bins = [
        ...
        np.linspace(-2, 4, 10 + 1),
        np.linspace(-0.1, 0.4, 10 + 1),
        np.linspace(0, 2, 10 + 1),
        np.linspace(-2, 2, 10 + 1),
    ]

    mms_props = MMSReader("StableJets.txt")

    DT_MMS = mms_props.read("DT") / 1.0e6
    ...
    var_list = [
        [DT_vlas_amax, DT_vlas_randt, DT_MMS],
        [Dn_vlas_amax, Dn_vlas_randt, Dn_MMS],
        [Dv_vlas_amax, Dv_vlas_randt, Dv_MMS],
        [Dpd_vlas_amax, Dpd_vlas_randt, Dpd_MMS],
        [DB_vlas_amax, DB_vlas_randt, DB_MMS],
    ]
    ...
    for row in range(5):
        for col in range(3):
            ...
            ax.hist(
                ...
                weights=weights,
                label="med:{:.2f}\nstd:{:.2f}".format(
                    np.median(var), np.std(var, ddof=1)
                ),
                histtype="step",
                bins=hist_bins[row],
            )
...
def DT_mach_comparison(fontsize=15, time_thresh=5):
    ...
    MMS_low = [
        mms_props_low.read(var) / mms_norm_list[mms_var_list.index(var)]
        for var in mms_var_list
    ]
    MMS_high = [
        mms_props_high.read(var) / mms_norm_list[mms_var_list.index(var)]
        for var in mms_var_list
    ]

    VLMax_low = [
        read_mult_runs(var, time_thresh, amax=True, runids=["AEA", "AEC"])[0]
        / vlas_norm_list[vlas_var_list.index(var)]
        for var in vlas_var_list
    ]
    VLMax_high = [
        read_mult_runs(var, time_thresh, amax=True, runids=["ABA", "ABC"])[0]
        / vlas_norm_list[vlas_var_list.index(var)]
        for var in vlas_var_list
    ]

    VLRand_low = [
        read_mult_runs(var, time_thresh, runids=["AEA", "AEC"])[0]
        / vlas_norm_list[vlas_var_list.index(var)]
        for var in vlas_var_list
    ]
    VLRand_high = [
        read_mult_runs(var, time_thresh, runids=["ABA", "ABC"])[0]
        / vlas_norm_list[vlas_var_list.index(var)]
        for var in vlas_var_list
    ]
    ...
    for row in range(5):
        for col in range(3):
            ...
            lab = [
                "med:{:.2f}\nstd:{:.2f}".format(
                    np.median(var_low), np.std(var_low, ddof=1)
                ),
                "med:{:.2f}\nstd:{:.2f}".format(
                    np.median(var_high), np.std(var_high, ddof=1)
                ),
            ]
...
def colorbar(mappable, ax_list):
    ...
    cax_list = [
        divider.append_axes("right", size="5%", pad=0.05) for divider in div_list
    ]
...
def get_timeseries(runid, start, stop, var_list, cellids):
    ...
    norm_list = [1.0e-9, 1.0e3, 1.0e-9, 1.0e6, 1.0e6, 1.0e6]
        ...
        for var in var_list:
            if var in v_vars:
                if len(var_list) == 1:
                    data_arr[itr] = (
                        vlsvobj.read_variable(
                            "v", operator=coord_vars[v_vars.index(var)], cellids=cellids
                        )
                        / norm_list[all_vars.index("v")]
                    )
                    time_arr[itr] = vlsvobj.read_parameter("t")
                else:
                    data_arr[var_list.index(var), itr] = (
                        vlsvobj.read_variable(
                            "v", operator=coord_vars[v_vars.index(var)], cellids=cellids
                        )
                        / norm_list[all_vars.index("v")]
                ...
                if len(var_list) == 1:
                    data_arr[itr] = (
                        vlsvobj.read_variable(
                            "B", operator=coord_vars[v_vars.index(var)], cellids=cellids
                        )
                        / norm_list[all_vars.index("B")]
                    )
                    time_arr[itr] = vlsvobj.read_parameter("t")
                else:
                    data_arr[var_list.index(var), itr] = (
                        vlsvobj.read_variable(
                            "B", operator=coord_vars[B_vars.index(var)], cellids=cellids
                        )
                        / norm_list[all_vars.index("B")]
                    )
                    time_arr[var_list.index(var), itr] = vlsvobj.read_parameter("t")
                ...
                if len(var_list) == 1:
                    data_arr[itr] = (
                        vlsvobj.read_variable(
                            var, operator="magnitude", cellids=cellids
                        )
                        / norm_list[all_vars.index(var)]
                    )
                    time_arr[itr] = vlsvobj.read_parameter("t")
                else:
                    data_arr[var_list.index(var), itr] = (
...
def hack_2019_fig2(runid, htw=60):
    ...
    time_ar, energy_ar, datamap = pt.plot.get_energy_spectrum(
        bulkpath,
        "bulk",
        "proton",
        filenr - htw - 100,
    ...
    time_mms = [
        t_mms,
        np.array([t_mms, t_mms, t_mms, t_mms]).T,
        np.array([t_mms, t_mms, t_mms, t_mms]).T,
        t_mms,
        t_mms,
        np.array([t_mms, t_mms]).T,
    ]
    ...
    for col in range(2):
        for row in range(6):
            if col == 0:
                ...
                ax = ax_list[row][col]
                ax.set_xlim(
                    float(filenr - htw - 100) / 2.0, float(filenr + htw + 100) / 2.0
                )
                if row == 3:
                    im = ax.pcolormesh(
                        time_ar,
                ...
                for m in range(len(ann_list)):
                    ax.annotate(
                        ann_list[m],
                        xy=(0.7 + m * 0.3 / len(ann_list), 0.05),
                        xycoords="axes fraction",
                        color=color_list[m],
                    )
            if col == 1:
                ...
                if row == 3:
                    ...
                    ax.yaxis.set_major_locator(MaxNLocator(nbins=5))
                ax.axvline(
                    mms_max_time, linestyle="dashed", linewidth=0.8, color="black"
                )
...
def hack_2019_fig1():
    ...
    vmax = [1.5, 4.5]
    pos_mms = [
        np.array([11.8709, -0.8539, -0.9172]),
        np.array([9.1060, -0.9715, -0.8508]),
    ]
    pos_vl = [
        np.array([11.81803436, -0.87607214, 0]),
        np.array([8.24222971, -0.87607214, 0]),
    ]
    ...
    for n in range(len(runids)):
        ...
        try:
            fileobj = open(
                wrkdir_DNR
                + "working/jets/events/{}/{}.events".format(runids[n], filenr[n]),
                "r",
            )
            contents = fileobj.read()
            fileobj.close()
        ...
        try:
            full_cells = np.loadtxt(
                wrkdir_DNR
                + "working/jets/Masks/{}/{}.mask".format(runids[n], filenr[n])
            ).astype(int)
        except IOError:
            full_cells = []
        ...
        pt.plot.plot_colormap(
            ...
            colormap="parula",
            cbtitle="nPa",
            external=h19_fig1_ext,
            pass_vars=["rho", "v", "CellID", "Pdyn"],
        )

        pt.plot.plot_colormap(
            filename=bulkpath + bulkname,
            outputfile=outputdir + outpfn_2[n],
            usesci=0,
            lin=1,
            boxre=boxre_2[n],
            expression=pc.expr_pdyn,
...
def h19_movie(runid, start, stop):
    ...
    boxre = [[6, 18, -8, 6], [6, 18, -6, 6], [6, 18, -8, 6], [6, 18, -6, 6]][
        runids_list.index(runid)
    ]
    ...
    for n in range(start, stop + 1):
        ...
        try:
            fileobj = open(
                wrkdir_DNR + "working/jets/events/{}/{}.events".format(runid, n), "r"
            )
            contents = fileobj.read()
            fileobj.close()
            jet_cells = list(map(int, contents.replace("\n", ",").split(",")[:-1]))
        ...
        try:
            full_cells = np.loadtxt(
                wrkdir_DNR + "working/jets/Masks/{}/{}.mask".format(runid, n)
            ).astype(int)
        except IOError:
            full_cells = []
        ...
        pt.plot.plot_colormap(
            filename=bulkpath + bulkname,
            outputfile=outputdir + str(n) + ".png",
            usesci=0,
            lin=1,
            boxre=boxre,
            expression=pc.expr_pdyn,
        ...
        pt.plot.plot_colormap(
            filename=bulkpath + bulkname,
            outputfile=outputdir + "zoom/" + str(n) + ".png",
            usesci=0,
            lin=1,
            boxre=[10, 14, -3, 1],
            expression=pc.expr_pdyn,
...
def h19_fig1_ext(ax, XmeshXY, YmeshXY, pass_maps):
    ...
    # full_cont = ax.contour(XmeshXY,YmeshXY,full_mask,[0.5],linewidths=0.8,colors="magenta") # Contour of full mask
    p_cont = ax.contour(
        XmeshXY, YmeshXY, plas_mask, [0.5], linewidths=0.6, colors="magenta"
    )
    jet_cont = ax.contour(
        XmeshXY, YmeshXY, jet_mask, [0.5], linewidths=0.8, colors="black"
    )  # Contour of jets
...
def h19_extra_1(runid, jetid):
    ...
    ax_list[0].set_title(
        "Runid: {},  Jetid: {}".format(runid, jetid), fontsize=20, pad=10
    )
    ax_list2[0].set_title(
        "Runid: {},  Jetid: {}".format(runid, jetid), fontsize=20, pad=10
    )
    ...
    fig2.savefig(
        homedir + "Figures/hackathon_timeseries/{}_{}_time.png".format(runid, jetid)
    )
    plt.close(fig)
    plt.close(fig2)
    ...
    fig2.savefig(
        homedir
        + "Figures/hackathon_timeseries/{}_{}_size_time.png".format(runid, jetid)
    )
    plt.close(fig)
    plt.close(fig2)
...
def get_SEA(
    var_list, centering="A", runids=["ABA", "ABC", "AEA", "AEC"], time_thresh=5
):
    ...
    for n in range(1, 3000):
        for runid in runids:
            ...
            for var in var_list:
                ...
                res_arr = np.interp(
                    epoch_arr, xdist_arr, var_arr, left=np.nan, right=np.nan
                )
...
def hack_2019_fig78(time_thresh=5):
    ...
    epoch_arr, SEA_mean_list_7_all, SEA_std_list_7_all, count_7_all = get_SEA(
        var_list_7, time_thresh=time_thresh
    )
    epoch_arr, SEA_mean_list_8_all, SEA_std_list_8_all, count_8_all = get_SEA(
        var_list_8, time_thresh=time_thresh
    )

    epoch_arr, SEA_mean_list_7_ABA, SEA_std_list_7_ABA, count_7_ABA = get_SEA(
        var_list_7, time_thresh=time_thresh, runids=["ABA"]
    )
    epoch_arr, SEA_mean_list_7_ABC, SEA_std_list_7_ABC, count_7_ABC = get_SEA(
        var_list_7, time_thresh=time_thresh, runids=["ABC"]
    )
    epoch_arr, SEA_mean_list_7_AEA, SEA_std_list_7_AEA, count_7_AEA = get_SEA(
        var_list_7, time_thresh=time_thresh, runids=["AEA"]
    )
    epoch_arr, SEA_mean_list_7_AEC, SEA_std_list_7_AEC, count_7_AEC = get_SEA(
        var_list_7, time_thresh=time_thresh, runids=["AEC"]
    )

    epoch_arr, SEA_mean_list_8_ABA, SEA_std_list_8_ABA, count_8_ABA = get_SEA(
        var_list_8, time_thresh=time_thresh, runids=["ABA"]
    )
    epoch_arr, SEA_mean_list_8_ABC, SEA_std_list_8_ABC, count_8_ABC = get_SEA(
        var_list_8, time_thresh=time_thresh, runids=["ABC"]
    )
    epoch_arr, SEA_mean_list_8_AEA, SEA_std_list_8_AEA, count_8_AEA = get_SEA(
        var_list_8, time_thresh=time_thresh, runids=["AEA"]
    )
    epoch_arr, SEA_mean_list_8_AEC, SEA_std_list_8_AEC, count_8_AEC = get_SEA(
        var_list_8, time_thresh=time_thresh, runids=["AEC"]
    )
    ...
    for col in range(4):
        ...
        if col != 3:
            ax.fill_between(
                epoch_arr, SEA_mean - SEA_std, SEA_mean + SEA_std, alpha=0.20
            )
        ...
        ax.annotate(
            fig7_labs[col], fig7_locs[col], xycoords="axes fraction", fontsize=20
        )
    ...
    for col in range(6):
        ...
        ax.annotate(
            fig8_labs[col], fig8_locs[col], xycoords="axes fraction", fontsize=20
        )
    ...
    ax_list_8[0].annotate(
        "LM30", xy=(0.5, 1.1), xycoords="axes fraction", color=jx.orange, fontsize=20
    )
    ax_list_8[0].annotate(
        "LM05",
        xy=(0.5 + 0.1, 1.1),
...
def hack_2019_fig9(time_thresh=5, nbins=10):
    ...
    bin_list = [
        np.linspace(0, 8, nbins + 1),
        np.linspace(0, 7, nbins + 1),
        np.linspace(0, 1, nbins + 1),
    ]

    vlas_high = [
        read_mult_runs(var, time_thresh, runids=["ABA", "ABC"], amax=False)
        for var in var_list
    ]
    vlas_low = [
        read_mult_runs(var, time_thresh, runids=["AEA", "AEC"], amax=False)
        for var in var_list
    ]
...
def make_transient_timeseries(runid, jetid, transient="jet"):
    ...
    ax_list[0].set_title(
        "Run: {} ID: {} Type: {}".format(runid, jetid, transient), fontsize=20
    )
