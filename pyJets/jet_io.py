...
class Transient:
    ...
    def jetprops_write(self, start):
        ...
        if self.transient != "slamsjet":
            ...
            if self.times[-1] - self.times[0] < 4.5:
                print(
                    "Transient {} is not SLAMSJET, propfile not written!".format(
                        self.ID
                    )
                )
                return None
            ...
            bsp_birth, bsp_death = [
                jx.bow_shock_jonas(self.runid, int(t[0] * 2))[::-1],
                jx.bow_shock_jonas(self.runid, int(t[-1] * 2))[::-1],
            ]
            x_bs = [
                np.polyval(jx.bow_shock_jonas(self.runid, int(t[n] * 2))[::-1], y[n])
                for n in range(len(y))
            ]
            t_crossing = t[np.argmin(np.abs(np.array(x) - np.array(x_bs)))]
            bsx_birth, bsx_death = [
                np.polyval(bsp_birth, y_birth),
                np.polyval(bsp_death, y_death),
            ]
            if (
                t_crossing - t[0] >= 1.5
                and t[-1] - t_crossing >= 4.5
                and x_birth >= bsx_birth
                and x_death <= bsx_death
            ):
                propfile_write(
                    self.runid,
                    start,
...
def jet_creator(
    ...
    for file_nr in range(start, stop + 1):
        ...
        if maskfile:
            ...
            jet_msk, slams_msk, slamsjet_msk = ja.mask_maker(
...
def jet_maker(
    ...
    for file_nr in range(start, stop + 1):
        ...
        if maskfile:
            ...
            msk = ja.make_cust_mask_opt(
                file_nr, runid, 180, boxre, avgfile, transient=transient
            )
...
def calc_event_props(
    ...
    if vlsvobj.check_variable("proton/rho"):
        rho, v, B, T, cellids, beta, TParallel, TPerpendicular = [
            np.array(vlsvobj.read_variable(s, cellids=cells), ndmin=1)
            for s in var_list_alt
        ]
        (
            rho_sheath,
            v_sheath,
            ...
            for s in sheath_list_alt
        ]
    else:
        rho, v, B, T, cellids, beta, TParallel, TPerpendicular = [
            np.array(vlsvobj.read_variable(s, cellids=cells), ndmin=1) for s in var_list
        ]
        (
            rho_sheath,
            v_sheath,
            ...
            np.array(vlsvobj.read_variable(s, cellids=sheath_cells), ndmin=1)
            for s in sheath_list
        ]

    pr_rhonbs = np.array(
        vlsvobj.read_variable("RhoNonBackstream", cellids=ssh_cells), ndmin=1
    )
    pr_PTDNBS = np.array(
        vlsvobj.read_variable("PTensorNonBackstreamDiagonal", cellids=ssh_cells),
        ndmin=1,
    )
            ...
            for b in y_r:
                for c in z_r:
                    neighbors = np.append(
                        neighbors,
                        int(
                            vlsvobj.get_cell_neighbor(
                                cellid=n, offset=[a, b, c], periodic=[0, 0, 0]
                            )
                        ),
                    )
...
def get_neighbors(vlsvobj, c_i, neighborhood_reach=[1, 1, 0]):
    ...
    return get_neighbors_asym(
        vlsvobj, c_i, neighborhood_reach=[-xn, xn, -yn, yn, -zn, zn]
    )
...
def sort_jets_2(
    ...
    while cells.size != 0:
        ...
        curr_event = np.intersect1d(
            cells, jx.get_neighs(runid_g, curr_event, neighborhood_reach)
        )
    ...
    for event in events_unsrt:
        ...
        for bs_event in bs_events:
            ...
            if check_threshold(bs_event, event, threshold):
                ...
                # Create new jet object
                jetobj_list.append(
                    Transient(curr_id, runid, float(start) / 2, transient=transient)
                )
                    ...
                    if event not in curr_jet_temp_list:
                        ...
                        if check_threshold(jetobj.cellids[-1], event, threshold):
                            if transient == "slamsjet" and sj_fulltrack:
                                ...
                                jetobj.cellids.append(event)
                                jetobj.props.append(
                                    props_unsrt[events_unsrt.index(event)]
                                )
                                jetobj.times.append(float(n) / 2)
                                # print("Updated jet "+jetobj.ID)
                    ...
                    if event not in curr_jet_temp_list:
                        ...
                        if check_threshold(jetobj.cellids[-2], event, threshold):
                            ...
                            if transient == "slamsjet" and sj_fulltrack:
                                ...
                                jetobj_new = Transient(
                                    curr_id, runid, float(n) / 2, transient=transient
                                )
                                jetobj_new.meta.append("splinter")
                                jetobj_new.cellids.append(event)
                                jetobj_new.props.append(
                                    props_unsrt[events_unsrt.index(event)]
                                )
                                jetobj_list.append(jetobj_new)
                                curr_jet_temp_list.append(event)
            ...
            if event not in curr_jet_temp_list:
                ...
                for bs_event in bs_events:
                    ...
                    if check_threshold(bs_event, event, threshold):
                        ...
                        jetobj_list[-1].cellids.append(bs_event)
                        jetobj_list[-1].cellids.append(event)
                        jetobj_list[-1].props.append(
                            bs_props[bs_events.index(bs_event)]
                        )
                        jetobj_list[-1].props.append(
                            props_unsrt[events_unsrt.index(event)]
                        )
