...
def fcs_jet_histogram(transient="jet", weight_by_run=False, magt=1.5):
    ...
    if transient == "slams":
        bins_list = [
            np.linspace(-1.5, 1.5, 10 + 1),
            np.linspace(-0.1, 0.3, 10 + 1),
    ...
    if weight_by_run:
        ...
        for idx, runid in enumerate(runids):
            ...
            jet_ids = np.union1d(sj_ids, non_sj_ids).astype(int)
            if transient == "slams":
                jet_ids = fcs_ids
            jet_counts[idx] = jet_ids.size
            for jet_id in jet_ids:
                trans_obj = jio.PropReader(
                    str(jet_id).zfill(5), runid, 580, transient=transient
                )
                jet_Dn[idx] = np.append(
                    jet_Dn[idx],
                    trans_obj.read_at_randt("Dn") / ja.sw_normalisation(runid, "Dn"),
                )
                jet_Dv[idx] = np.append(
                    jet_Dv[idx],
                    trans_obj.read_at_randt("Dv") / ja.sw_normalisation(runid, "Dv"),
                )
                jet_Dpd[idx] = np.append(
                    jet_Dpd[idx],
                    trans_obj.read_at_randt("Dpd") / ja.sw_normalisation(runid, "Dpd"),
                )
                jet_DB[idx] = np.append(
                    jet_DB[idx],
                    trans_obj.read_at_randt("DB") / ja.sw_normalisation(runid, "DB"),
                )
                jet_DT[idx] = np.append(
                    jet_DT[idx],
                    trans_obj.read_at_randt("DT") / ja.sw_normalisation(runid, "DT"),
                )

                jet_dur[idx] = np.append(
                    jet_dur[idx], trans_obj.read_at_randt("duration")
                )
                jet_tsiz[idx] = np.append(
                    jet_tsiz[idx], trans_obj.read_at_randt("size_tan")
                )
                jet_srat[idx] = np.append(
                    jet_srat[idx], trans_obj.read_at_randt("size_ratio")
                )
    else:
        jet_counts = 0
        ...
        data_arr = np.array(
            [jet_Dn, jet_Dv, jet_Dpd, jet_DB, jet_DT, jet_dur, jet_tsiz, jet_srat]
        )
    ...
    fig.savefig(
        wrkdir_DNR
        + "Figures/thesis/{}_stats_runweight_{}_magt_{}.png".format(
            transient, weight_by_run, magt
        )
    )
    plt.close(fig)
