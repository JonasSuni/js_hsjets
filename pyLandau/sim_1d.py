...
def grad_false(f):
    ...
    return (2*f-np.append(f[0],f)[:-1]-np.append(f,f[-1])[1:])/2
...
def divr_false(flux,r):
    ...
    return (2*flux-np.append(flux[0],flux)[:-1]-np.append(flux,flux[-1])[1:])/(2*r**2)
...
def sim_nv_rk4(n0=100,v0=100,T0=1,t=10000,dt=1,rmax=10,dr=0.1,figname="unnamed",animate=True,cpf=1000):
    ...
    while not stop:
        ...
        k1v = dt*(-v*grad(v)-T0*grad(n)/n)
        
        k2n = dt*(-divr((v+k1v/2)*(n+k1n/2)*r**2,r))
        k2v = dt*(-(v+k1v/2)*grad(v+k1v/2)-T0*grad(n+k1n/2)/(n+k1n/2))
        
        k3n = dt*(-divr((v+k2v/2)*(n+k2n/2)*r**2,r))
        k3v = dt*(-(v+k2v/2)*grad(v+k2v/2)-T0*grad(n+k2n/2)/(n+k2n/2))
        
        k4n = dt*(-divr((v+k3v)*(n+k3n)*r**2,r))
        k4v = dt*(-(v+k3v)*grad(v+k3v)-T0*grad(n+k3n)/(n+k3n))
...
def sim_nvT_rk4(n0=100,v0=100,T0=0.1,qpar=0.1,qperp=0.1,t=10000,dt=1,rmax=10,dr=0.1,figname="unnamed",animate=True,cpf=1000):
    ...
    while not stop:
        ...
        k1n = dt*(-divr(v*n*r**2,r))
        k1v = dt*(-v*grad(v)-Tpar*grad(n)/n)
        k1T1 = -dt*(divr(v*n*Tpar*r**2,r)+2*n*Tpar*grad(v))
        k1T2 = -dt*(divr(v*n*Tperp*r**2,r)+n*Tperp*divr(v,r)-n*Tperp*grad(v))
        
        k2n = dt*(-divr((v+k1v/2)*(n+k1n/2)*r**2,r))
        k2v = dt*(-(v+k1v/2)*grad(v+k1v/2)-(Tpar+k1T1/2)*grad(n+k1n/2)/(n+k1n/2))
        k2T1 = -dt*(divr((v+k1v/2)*(n+k1n/2)*(Tpar+k1T1/2)*r**2,r)+2*(n+k1n/2)*(Tpar+k1T1/2)*grad(v+k1v/2))
        k2T2 = -dt*(divr((v+k1v/2)*(n+k1n/2)*(Tperp+k1T2/2)*r**2,r)+(n+k1n/2)*(Tperp+k1T2/2)*divr((v+k1v/2),r)-(n+k1n/2)*(Tperp+k1T2/2)*grad(v+k1v/2))

        k3n = dt*(-divr((v+k2v/2)*(n+k2n/2)*r**2,r))
        k3v = dt*(-(v+k2v/2)*grad(v+k2v/2)-(Tpar+k2T1/2)*grad(n+k2n/2)/(n+k2n/2))
        k3T1 = -dt*(divr((v+k2v/2)*(n+k2n/2)*(Tpar+k2T1/2)*r**2,r)+2*(n+k2n/2)*(Tpar+k2T1/2)*grad(v+k2v/2))
        k3T2 = -dt*(divr((v+k2v/2)*(n+k2n/2)*(Tperp+k2T2/2)*r**2,r)+(n+k2n/2)*(Tperp+k2T2/2)*divr((v+k2v/2),r)-(n+k2n/2)*(Tperp+k2T2/2)*grad(v+k2v/2))

        k4n = dt*(-divr((v+k3v)*(n+k3n)*r**2,r))
        k4v = dt*(-(v+k3v)*grad(v+k3v)-(Tpar+k3T1)*grad(n+k3n)/(n+k3n))
        k4T1 = -dt*(divr((v+k3v)*(n+k3n)*(Tpar+k3T1)*r**2,r)+2*(n+k3n)*(Tpar+k3T1)*grad(v+k3v))
        k4T2 = -dt*(divr((v+k3v)*(n+k3n)*(Tperp+k3T2)*r**2,r)+(n+k3n)*(Tperp+k3T2)*divr((v+k3v),r)-(n+k3n)*(Tperp+k3T2)*grad(v+k3v))
