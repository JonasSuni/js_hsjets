...
def ahfmake(file_number,runid,halftimewidth):
    # creates temporary vlsv file with new variable: ratio of dynamic pressure
    # and time-averaged dynamic pressure
    ...
    # open vlsv files for reading and writing
    vlsvreader = pt.vlsvfile.VlsvReader(file_path)
    vlsvwriter = pt.vlsvfile.VlsvWriter(vlsvReader=vlsvreader,file_name="VLSV/temp_archerhorbury.vlsv")

    rho = vlsvreader.read_variable("rho")
    v = vlsvreader.read_variable("v")

    # read cellids for the purpose of sorting variables
    origid = vlsvreader.read_variable("CellID")
    sorigid = vlsvreader.read_variable("CellID")
    sorigid.sort()
    ...
    # initialise the time average of the dynamic pressures
    tpdynavg = np.zeros(len(pdyn))

    for n in timerange:

        # exclude file number from average
        if n == file_number:
            continue
    ...
    # calculate time average of dynamic pressures
    tpdynavg /= len(timerange)-1
    
    # sort dynamic pressure and velocity magnitudes
    spdyn = pdyn[origid.argsort()]
    svmag = np.linalg.norm(v,axis=-1)[origid.argsort()]


    # calculate the ratio of the dynamic pressure and its time average
    tapdyn = spdyn/tpdynavg

    # write the new variables to the writer file
    vlsvwriter.write(data=tapdyn,name="tapdyn",tag="VARIABLE",mesh="SpatialGrid")
    vlsvwriter.write(data=svmag,name="svmag",tag="VARIABLE",mesh="SpatialGrid")
    vlsvwriter.write(data=sorigid,name="CellID",tag="VARIABLE",mesh="SpatialGrid")
...
def kfmake(file_number,runid,halftimewidth):
    # creates temporary vlsv file with new variable: ratio of density and time-averaged density
    ...
    # open vlsv files for reading and writing
    vlsvreader = pt.vlsvfile.VlsvReader(file_path)
    vlsvwriter = pt.vlsvfile.VlsvWriter(vlsvReader=vlsvreader,file_name="VLSV/temp_karlsson.vlsv")
    ...
    # initialise the time average of the density
    trhoavg = np.zeros(len(rho))
    ...
    # calculate time average of densities
    trhoavg /= len(timerange)-1
    
    # sort densities etc.
    srho = rho[origid.argsort()]

    # calculate the ratio of the density and its time average
    tarho = srho/trhoavg
